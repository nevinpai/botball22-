...
void dual_servos(int amt, int inc){ // pos amt for forward, neg amt for back (in servo ticks), inc is the increment
    if(amt<0) int forward = 0;
    else int forward = 1;
    int lserv = 0;
    int count = 0;
    while(count<amt && get_servo_position(1)>0 && get_servo_position(1)<2047){
      if(forward == 0){
        motor(0, -1000);
        if(amt-count<inc) inc = (amt-count)-1;
        set_servo_position(get_servo_position(1)-inc);
      }
      else{
        motor(0, 1000);
        if(amt-count<inc) inc = (amt-count)-1;
